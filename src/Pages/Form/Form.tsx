...
export default function Form() {
    ...
    const { register: todoRegister, handleSubmit: todoSubmit, setValue, formState: { errors: todoError } } = useForm<ITask>({
    ...
    const onSubmit = (data: ITask) => {
        ...
        dispatch(todoSliceAction.addToDo(data));
        setValue('title', '');
    ...
    return (
        <div className='container'>
            ...
            <form onSubmit={todoSubmit(onSubmit)}>
                ...
                <input {...todoRegister('title')} type='text' className='form-control' />

                {todoError.title && <p>{todoError.title.message}</p>}

                <div className="mb-3 form-check">
                    <input {...todoRegister('complete')} type='checkbox' className='form-check-input' id="checkbox" />
                    <label className='form-check-label' htmlFor="checkbox">Complete</label>
